...
namespace PolyPerfect
{
    ...
    public class Animal_WanderScript : Common_WanderScript 
    {
        ...
        [SerializeField] private FeastState[] feastStates;
        ...
        float feastUpdateTime;
        Vector3 deadPreyPosition;
        DeleteMonster monsterDisappear;
        ...
        private void Awake()
        {
            ...
            monsterDisappear = GetComponent<DeleteMonster>();
        }
        ...
        IEnumerable<AIState> AllStates
        {
            get
            {
                ...
                foreach (var item in attackingStates)
                    yield return item;
                foreach (var item in feastStates)
                    yield return item;
        ...
        void Update()
        {
            ...
            {
                if (!attackTarget || attackTarget.CurrentState == WanderState.Dead || nonAgressiveTowards.Contains(attackTarget.species))
                {
            ...
            switch (CurrentState)
            {
                ...
                case WanderState.Chase:
                    if (!primaryPrey || primaryPrey.CurrentState == WanderState.Dead || nonAgressiveTowards.Contains(primaryPrey.species))
                    {
                        primaryPrey = null;
                ...
                case WanderState.Feast:
                    targetPosition = deadPreyPosition;
                    if (Time.time >= feastUpdateTime)
                    {
                        SetState(WanderState.Idle);
                        UpdateAI();
                    }
        ...
        public void UpdateAI()
        {
            ...
            var wasTargetDead = false;
            if (primaryPrey)
            {
                if (primaryPrey.CurrentState == WanderState.Dead)
                {
                    deadPreyPosition = primaryPrey.transform.position;
                    primaryPrey = null;
                    wasTargetDead = true;
                }
                else if (nonAgressiveTowards.Contains(primaryPrey.species))
                    primaryPrey = null;
                else
                {
            ...
            if (shouldAttack)
                ...
                SetState(WanderState.Feast);
            else if (CurrentState != WanderState.Idle && CurrentState != WanderState.Wander && CurrentState != WanderState.Feast)
                SetState(WanderState.Idle);
        ...
        void HandleBeginFeast()
        {
            var feastState = Random.Range(0, feastStates.Length);
            //turnSpeed = 120f;
            feastUpdateTime = Time.time + Random.Range(feastStates[feastState].minStateTime, feastStates[feastState].maxStateTime);
            ClearAnimatorBools();
            TrySetBool(feastStates[feastState].animationBool, true);
            moveSpeed = 3f;
            //attackingEvent.Invoke();
        }
